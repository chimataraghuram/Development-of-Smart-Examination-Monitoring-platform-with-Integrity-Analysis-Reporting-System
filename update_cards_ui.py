...
# Inject CSS before </style> in the head
content = content.replace('/* ===== BOTTOM PANELS (Quick Actions etc.) ===== */', css_updates + '\n/* ===== OLD BOTTOM PANELS ===== */')
...
# We need to replace the old bottom panels with new_panels_html
start_idx = content.find('        <!-- ===== BOTTOM PANELS (Quick Actions, Recent Events, Timeline) ===== -->')
end_idx = content.find('        </div>', start_idx) + 14  # End of bottom-panels div
end_idx = content.find('</div>', end_idx) + 6 # go up to the end of bottom-panels container

content = content[:start_idx] + new_panels_html + content[end_idx:]

# JS Updates for populateBottomPanels
js_new = """
            if (recentList && typeof window._allEvents !== 'undefined') {
                ...
                if (suspicious.length > 0) {
                    html += '<div class="card-row-list" style="justify-content: flex-start;">';
